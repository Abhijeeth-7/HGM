...
export class CodeGenerationService {
  actionGestureId = 0;
  processNameMap = new Map();
  generatedScript: string = '';
  modes = [1, 2, 3, 4, 5];

  constructor(private dataService: DataService) {
    this.dataService.appNameData.forEach((app) =>
      this.processNameMap.set(app.title, app.value)
    );
  }
  ...
  generateApplicationBlocks(records: any) {
    ...
    appNames.forEach((appName: string) => {
      ...
      if (
        ...
        result += `
            if ApplicationName == '${this.processNameMap.get(appName)}':
                ${ifBlocks}
            `;
      }
    });
